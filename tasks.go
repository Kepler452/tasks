...
func New[T any]() *Scheduler[T] {
	s := &Scheduler[T]{}
	s.tasks = make(map[string]*Task[T])
	return s
}
...
//		// Do stuff
//	}
func (schd *Scheduler[T]) Add(t *Task[T]) (string, error) {
	id := xid.New()
...
//		// Do stuff
//	}
func (schd *Scheduler[T]) AddWithID(id string, t *Task[T]) error {
...
func (schd *Scheduler[T]) Del(name string) {
...
// panics.
func (schd *Scheduler[T]) Lookup(name string) (*Task[T], error) {
	schd.RLock()
	defer schd.RUnlock()
...
// The returned task should be treated as read-only, and not modified outside of this package. Doing so, may cause
// panics.
func (schd *Scheduler[T]) Tasks() map[string]*Task[T] {
	schd.RLock()
	defer schd.RUnlock()
	m := make(map[string]*Task[T])
...
// time specified.
func (schd *Scheduler[T]) scheduleTask(t *Task[T]) {
	select {
...
func (schd *Scheduler[T]) execTask(t *Task[T]) {
	go func() {
		if t.MaxTimes == 0 {
			if t.currentTime > 0 {
				return
		...
		if err != nil && errors.Is(err, ErrStopTask) {
			defer schd.Del(t.id)
		}
	}()
	if !t.RunOnce {
